...
SMART_ANSWER_MIN_SCORE = 68
SMART_ANSWER_CLOSE_DELTA = 12
SMART_ANSWER_MAX_ITEMS = 4
...
ACTIVITY_ACTION_LABELS = {
    "support_requested": "درخواست ارتباط با پشتیبانی از چت",
...
COMMON_TYPO_REPLACEMENTS = {
    ...
    "سامنانه": "سامانه",
    "ارصال": "ارسال",
    "ارسال نمیشه": "ارسال نمی شود",
    "ذخیره نمیشه": "ذخیره نمی شود",
    "زخیره": "ذخیره",
    "دخیره": "ذخیره",
    "گزارس": "گزارش",
    "گزارشات": "گزارش",
    "کاربرجدید": "کاربر جدید",
    "سالمالی": "سال مالی",
...
def normalize_text(value: Any) -> str:
    """نرمال‌سازی بهتر برای تحمل غلط املایی و نگارشی فارسی."""
    if value is None:
        return ""
    ...
    text_value = text_value.translate(CHAR_MAP)
    text_value = text_value.lower()

    text_value = re.sub(r"[\u200c\u200f\u200e]", " ", text_value)

    for wrong_value, correct_value in COMMON_TYPO_REPLACEMENTS.items():
        text_value = text_value.replace(wrong_value, correct_value)

    text_value = re.sub(r"(.)\1{2,}", r"\1\1", text_value)
    text_value = re.sub(r"[^\w\sآ-یa-z0-9]", " ", text_value, flags=re.UNICODE)
...
def init_db() -> None:
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                """
                IF OBJECT_ID(N'dbo.chat_alerts', N'U') IS NULL
                BEGIN
                    CREATE TABLE dbo.chat_alerts (
                        id INT IDENTITY(1,1) PRIMARY KEY,
                        user_id INT NULL,
                        ...
                        created_at NVARCHAR(80) NOT NULL,
                        seen_at NVARCHAR(80) NULL,
                        resolved_at NVARCHAR(80) NULL
                    );
                END
                """
            )
...
def load_active_qa_index() -> List[Dict[str, Any]]:
    ...
    return QA_INDEX
def detect_message_intent(raw_message: str, normalized_message: str = "") -> str:
    raw_text = str(raw_message or "").strip()
    normalized_text = normalized_message or normalize_text(raw_text)
    ...
    for keyword in ERROR_INTENT_KEYWORDS:
        normalized_keyword = normalize_text(keyword)
        if normalized_keyword and normalized_keyword in normalized_text:
            error_score += 2
        elif keyword.lower() in raw_lower:
            error_score += 2
    ...
    for keyword in SUPPORT_INTENT_KEYWORDS:
        normalized_keyword = normalize_text(keyword)
        if normalized_keyword and normalized_keyword in normalized_text:
            support_score += 2
        elif keyword.lower() in raw_lower:
    ...
    if re.search(r"\b\d{3,}\b", raw_text) and error_score > 0:
        error_score += 1

    if "\n" in raw_text and error_score > 0:
        error_score += 1
...
def detect_message_intent(raw_message: str, normalized_message: str = "") -> str:
    ...
    # اگر متن شامل عددهای طولانی، کد خطا، شماره سند یا خط چندبخشی باشد،
    # احتمال خطا بودن بیشتر می‌شود.
    if re.search(r"\b\d{3,}\b", raw_text) and error_score > 0:
        error_score += 1
    ...
    # اولویت با خطاست، چون متن خطاها گاهی شامل کلمات عمومی هم هستند.
    if error_score >= 2:
        return "error"
    ...
    # هر چیزی که خطا، ویدیو یا پشتیبانی نبود،
    # فعلاً سؤال عادی محسوب می‌شود و وارد FAQ می‌شود.
    return "faq"
...
def search_best_answers(normalized_question: str) -> List[Dict[str, Any]]:
    ...
    if best_score < SMART_ANSWER_MIN_SCORE:
        return []
    ...
    for scored in scored_items:
        ...
        if score < SMART_ANSWER_MIN_SCORE:
            continue

        if (best_score - score) > SMART_ANSWER_CLOSE_DELTA:
            continue

        selected.append(scored)

        if len(selected) >= SMART_ANSWER_MAX_ITEMS:
            break

    return selected
def build_smart_answer(raw_question: str, best_matches: List[Dict[str, Any]]) -> str:
    if not best_matches:
        return ""
    ...
    if len(unique_items) == 1:
        return unique_items[0]["answer"]

    lines = [
        "چند پاسخ مرتبط در پایگاه دانش پشتیبانی پیدا شد. جمع‌بندی قابل اتکا:",
        "",
    ]
    ...
    lines.append(
        "این پاسخ فقط بر اساس اطلاعات موجود در پایگاه دانش مدریک ساخته شده است. "
        "اگر دقیقاً مشکل شما را حل نکرد، سؤال را با جزئیات بیشتر بپرسید."
    )
...
def get_exact_knowledge_answer(normalized_question: str):
    ...
    for row in rows:
        db_question = str(row["question"] or "").strip()
        db_normalized_question = normalize_text(db_question)

        if db_normalized_question == normalized_question:
            return {
                "question": db_question,
                "answer": str(row["answer"] or "").strip(),
                "category": str(row["category"] or "عمومی").strip() or "عمومی",
            }
...
@app.route("/admin/chat-alerts", methods=["GET"])
@support_manager_required_page
def admin_chat_alerts():
    rows = db_fetchall(
        """
        SELECT TOP 300
            id,
        ...
        alerts.append(
            {
                ...
                "status": row["status"] or "new",
                "status_label": "جدید" if row["status"] == "new" else "بررسی‌شده",
                "created_at": format_datetime(row["created_at"]),
            }
        )
